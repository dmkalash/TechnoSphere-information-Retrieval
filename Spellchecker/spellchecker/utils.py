...
def get_candidates(query, trie, letter_dict, bigramm_words_dict,
                   max_size=CAND_SIZE, max_ans_size=FIX_MAX_ANS_SIZE, alpha=FIX_ALPHA, gamma=FIX_GAMMA, return_query=True):
    words = prepare_query(query)
    word_candidates = []
    for word in words:
        # TODO: проигнорировать числа
        sugg = fuzzy_search(trie, word, letter_dict, alpha=alpha,
                            max_heap_size=FIX_MAX_HEAP_SIZE, max_ans_size=max_ans_size)
    ...
    if return_query: # TODO: что-то странное здесь в join
        res = []
        for key in ans:
...
def get_model(query_dict, trie, letter_dict, bigramm_words_dict, words_prior_dict,
              stop_iter=100):
    X, y = get_features_train(query_dict, trie, letter_dict, bigramm_words_dict, words_prior_dict,
                              stop_iter=stop_iter)
    scaler = SS()
    scaler.fit(X)
...
def predict(query, cand, model, scaler, query_dict, words_prior_dict, bigramm_words_dict):
    ...
    #print(query_dict_val, was_fixed)

    if was_fixed:
        return True

    X_test = get_features_test(query, cand, tokens, query_dict, words_prior_dict, bigramm_words_dict)
    if X_test[0][0] * X_test[0][1] == 0:
        return True
...
def get_features_train(query_dict, trie, letter_dict, bigramm_words_dict, words_prior_dict,
...
def get_features_test(query, cand, tokens, query_dict, words_prior_dict, bigramm_words_dict,
                        teta=FIX_TETA):
...
def get_features_train(query_dict, trie, letter_dict, bigramm_words_dict, words_prior_dict,
    ...
    with open(fname) as fin:
        ...
        for i, line in enumerate(fin):
            ...
            corr_score, corr_query = get_candidates(query, trie, letter_dict, bigramm_words_dict,
                        max_size = CAND_SIZE, max_ans_size = FIX_MAX_ANS_SIZE, alpha = FIX_ALPHA, gamma = FIX_GAMMA)[0]
            corr_tokens = corr_query.split()
            bigramm_prior = 0
            for word1, word2 in zip(corr_tokens, corr_tokens[1:]):
                bigramm_prior += math.log(1 + bigramm_words_dict.get('_'.join((word1, word2)), 0))
            bigramm_prior += teta * math.log(1 + LEN_WORDS_DICT * words_prior_dict.get(corr_tokens[-1], 0)) # len(words_dict)
...
def get_features_test(line, query_dict, trie, letter_dict, bigramm_words_dict, words_prior_dict,
                        teta=FIX_TETA):
    pair = line.split('\t')
