...
class Levenstein1:
    ...
    def get_distance(self, origin, fixed, alpha=1.05, coef=1):  # 10 TODO: вероятность
        ...
        for i in range(1, len(fixed) + 1):
            for j in range(1, len(origin) + 1):
                table[i, j] = min(table[i - 1, j] + self.weights.get('_' + fixed[i - 1], 0),
                                  table[i, j - 1] + self.weights.get(origin[j - 1] + '_', 0),
                                  table[i - 1, j - 1] + (origin[j - 1] == fixed[i - 1]) *
                                  self.weights.get(origin[j - 1] + fixed[i - 1], 0))
